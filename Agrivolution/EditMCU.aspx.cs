...
namespace Agrivolution
{
    ...
    public partial class EditMCU : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (IsPostBack)
            {
                DateTime start = Convert.ToDateTime(startSelect.Text);
                DateTime end = Convert.ToDateTime(endSelect.Text);
            ...
            else //page loads for the first time
            {
                ...
                try
                {
                    ...
                    {
                        SqlCommand com = new SqlCommand("select * from MCU where MCUId='" + MCUId + "'" + "AND UserName=@UserName", connect);
                        connect.Open();
                        com.Parameters.AddWithValue("@UserName", User.Identity.Name);
                        SqlDataReader read = com.ExecuteReader();
                        if (read.Read())
                        {
                            roomName.Text = read["Room"].ToString();
                            facilityName.Text = read["Facility"].ToString();
                            ...
                            cropType.Text = read["CropType"].ToString(); // populates croptype textbox
        ...
        protected void setMCUValues(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    connect.Open();
                    SqlCommand com = new SqlCommand("update MCU set Room=@Room, Facility=@Facility, FanStatus=@FanStatus, LightOn=@StartTime, LightOff=@EndTime, CropType=@CropType where MCUId=@initializerMCUId AND UserName=@UserName", connect);
                    com.Parameters.AddWithValue("@Room", roomName.Text);
                    com.Parameters.AddWithValue("@Facility", facilityName.Text);
